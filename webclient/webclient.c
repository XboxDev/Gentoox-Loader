...
const char *requestGET  = "GET /sourceforge/xbox-linux/resctoox.t00x HTTP/1.0\n\n";
const char *requestHEAD = "HEAD /sourceforge/xbox-linux/resctoox.t00x HTTP/1.0\n\n";
int i = 0, initrdSize = 0, kernelSize = 0, hLen = 0, contLen = 0, fraction = 0;
int head = 1, progCheck = 0, eoh = 0, fileLen = 0;
char *tempBuf = (u8*)INITRD_START;
char c[4], *appendLine = NULL, *header = NULL;
		...
		for (i = 0; i < q->len; i++) {
			...
			if(eoh == 1) {
				if(head == 0) {
					...
					fileLen++;
					tempBuf[fileLen] = '\0';
					if(fileLen > progCheck) {
						DisplayFlashProgressBar(fileLen,contLen,0xffff0000);
						progCheck += fraction;
					}
				}
			}
...
void httpc_init(int A, int B, int C, int D, int P) {
